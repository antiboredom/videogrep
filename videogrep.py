...
def findWholeWord(srch):
    return re.compile(r'\b({0})\b'.format(srch), flags=re.IGNORECASE).search

def search_line(line, search, searchtype):
    if searchtype == 're':
        return findWholeWord(search)(line)  #, re.IGNORECASE)
    elif searchtype == 'pos':
            ...
            if lines:
                ...
                for timespan in lines.keys():
                    ...
                    for term in searchTerms:					
                        if search_line(line, term, searchtype):
